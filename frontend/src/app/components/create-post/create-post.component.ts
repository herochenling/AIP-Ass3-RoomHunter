...
const URL = 'http://127.0.0.1:4000/upload/';
@Component({
...
export class CreatePostComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.postForm = this.formBuilder.group({
      ...
      rent: ['', [Validators.required, Validators.pattern(/^[0-9]+.?[0-9]*$/)]],
  ...
  onCreatedSubmit() {
    ...
    this.postService.createPost(
      ...
      this.rent, this.byUser, this.postDate, this.description, this.houseImage).subscribe(data => {
  ...
  public uploadFile() {
    ...
    this.uploadService.uploadFile(formData).subscribe(data => {
      if (data != undefined || data != null) {
        ...
        console.log(this.uploadService.getHouseImage());
        this.flashMessageService.showMessage((data as any).msg, false);
      } else {
