...
export class PostService {
    ...
    createPost(title, street, suburb, city, state, postCode, roomType, contactName,
        phone, rent, byUser, postDate, description, houseImage) {
        const post = {
            title: title,
            street: street,
            suburb: suburb,
     ...
     */
    deletePost(id) {
        return this.http.get(`${this.url}/posts/delete/${id}`);
    }

    /**
     * searcg post by username
     */
    ...
    searchPostByArea(city) {
        if (city == null || city == undefined) city = 'all';
    ...
    searchPostByPrice(price) {
        // get lowest and highest price from the price options
        const price_begin = parseFloat(price[0]);
        const price_end = parseFloat(price[price.length - 1]);
        let price_low = 0,
            price_high = 100000;
        if (price_begin == 1) {
            price_low = 99;
